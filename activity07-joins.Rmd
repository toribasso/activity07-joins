...
```{r importingactivitydata, message=FALSE}
...
**Response**: While Denny's does not have any locations outside of the US, La
Quinta does have many international locations.
...
**Response**: We could antijoin with Dennys using state, or we could filter out observations
for La Quinta where there is not a US state.
...
**Response**: There are no Denny locations outside of the US.
...
```{r antijoindenny}
anti_join(states, dennys, by = c("name" = "state"))
```
...
**Response**: Looks like DE or Deleware only has one location.
...
**Response**: The state of Maine or ME has the fewest LaQuinta locations.


Is this surprising to you? Why or why not?

**Response**: I am and I am not. I am surprised that the ones with smaller landmass
(Rhode Island, DC, ect) were not the ones with the fewest, but this could be due
...
DennyCount <- dennys %>%
  ...
  summarise(count=n()) %>%
  arrange(count)
states <- inner_join(states, DennyCount, by = c("abbreviation" = "state"))
states <- states %>% 
  mutate(location = (count / area))
```


Which states have the most Denny's locations per thousand square miles?

**Response**: DC, Rhode Island, and California.
...
```{r LaQuintaCount}
LaQuintaCount <- laquinta_us %>%
  group_by(state) %>%
  summarise(number=n()) %>%
  arrange(number)
statesLaQ <- inner_join(states,LaQuintaCount, by = c("abbreviation" = "state"))
statesLaQ <- statesLaQ %>% 
  mutate(locationLQ = (number / area))
```

**Response**: The most number per thousand miles is Rhode Island, Florida, and Connecticut.
